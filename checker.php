...
<body>
<?php
echo '<h1> Site <a href="http://'.$_SERVER["HTTP_HOST"].'" target="_blank">'.$_SERVER["HTTP_HOST"].'</a> checker. Ver. 2.0150423</h1>';
...
function filesBK() {
	...
	$indexrobotsfile = 'robots.txt';
	$indexrobotscont = file_get_contents($indexrobotsfile);
	$indexrobotsbackfile = "robots.txt_checker_autobackup";
		if (file_put_contents ($indexrobotsbackfile, $indexrobotscont)) {
			echo '<li>robots.txt backup created</li>';}
			else echo '<li style="color:red"><b>cant create robots.txt backup</b></li>';
		}
		else echo ('<li style="color:#993300">file robots.txt is not exist</li>');
			
	if (file_exists('sitemap.xml')) {
	$indexsitemapfile = 'sitemap.xml';
	$indexsitemapcont = file_get_contents($indexsitemapfile);
	$indexsitemapbackfile = "sitemap.xml_checker_autobackup";
		if (file_put_contents ($indexsitemapbackfile, $indexsitemapcont)) {
			echo '<li>sitemap.xml backup created</li>';}
			else echo '<li style="color:red"><b>cant create sitemap.xml backup</b></li>';
		}
		else echo ('<li style="color:#993300">file sitemap.xml is not exist</li>');
